...
import {
	DekstopDashboard,
	DesktopAddElectronics,
	DesktopEditProductHome,
} from './';
		...
		try {
			...
			if (response === -1) {
				Modal.error({
					title: 'Error',
					content:
						'Internal Server Error, Please check after sometime',
				});
			} else if (response === 0) {
				history.push('/');
			}
				...
				>
					...
					>
						<Menu.Item
							key="3"
							onClick={() =>
								history.push('/desktopSeller/electronics')
							}
						>
							Electronics
						</Menu.Item>
						<Menu.Item key="4">Fashion</Menu.Item>
						<Menu.Item key="5">Grocery</Menu.Item>
					</SubMenu>

					<Menu.Item
						icon={<EditOutlined />}
						onClick={() =>
							history.push('/desktopSeller/editProduct')
						}
						key="6"
					>
						Edit Product
					...
					<Menu.Item key="9" icon={<UserOutlined />} onClick={logout}>
						Logout
					</Menu.Item>
				</Menu>
			</Sider>
